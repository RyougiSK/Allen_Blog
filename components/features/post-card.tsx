  ...
  return (
    <Link href={`/blog/${post.slug}`} className="block group">
      <article className="rounded-lg border border-border bg-surface p-6 transition-colors hover:border-muted/50">
        ...
        <h3 className="text-lg font-semibold tracking-tight mb-2 group-hover:text-accent-warm transition-colors">
          {post.title}
        </h3>
        <p className="text-sm text-text-tertiary leading-relaxed mb-3">{excerpt}</p>
        {post.tags.length > 0 && (
